...
namespace Show_Element_Log_1
{
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			if (elements.Select(element => element.Name).Contains(input.Value))
			{
			...
			GiveSuggestions(engine, elements, input.Value);
		}
		...
		private void GiveSuggestions(IEngine engine, IEnumerable<IDmsElement> protocols, string input)
		{
			var card = new List<AdaptiveElement>
			{
				new AdaptiveTextBlock($"Could not find a element with name: '{input}'. Did you mean any of the following?") { Wrap = true },
			};

			var list = new Dictionary<string, ElementMatch>();
			foreach (var protocol in protocols)
			{
				list.Add(protocol.Name, new ElementMatch(input, protocol.Name));
			}
